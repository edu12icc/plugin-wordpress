<div class="wrap">
    <h2><?php _e( 'Modificar Plan de Adquisición', 'bcie' ); ?></h2>
    ...
    <form action="" method="post" class="formulario">
        ...
        <table class="form-table">
            <tbody>
              ...
              <!--  <tr class="row-">
                    ...
                    <td>
                      <select class="regular-text" class="" name="">
                        <?php
                          global $wpdb;
                          $resultados= $wpdb->get_results( "SELECT DISTINCT(LineName), CONCAT('(', CurrentCreditLine, ') - ', LineName) AS Detalle FROM wp_bcie_app_finishedprocess" );
                          foreach ( $resultados as $fila ){?>
                            <option value="<?php echo $fila->id?>"<?php
                            if ($idc == $fila->id) {
                              echo "selected";
                            }
                             ?>><?php echo $fila->LineName; $item->CurrentCreditLine; ?></option>
                          <?php } ?>
                      </select>
                    </td>
                </tr> -->
        ...
        <?php wp_nonce_field( '' ); ?>
        <?php submit_button( __( 'Modificar', 'bcie' ), 'primary', 'submit_plan' ); ?>
